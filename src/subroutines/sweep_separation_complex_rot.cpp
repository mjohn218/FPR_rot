	...
	while (it < maxit) {
		...
		for (c = 0; c < csize; c++) {
			...
			for (i = 0; i < ncross[p1]; i++) {
				...
				if(k1!=k2){
				  ...
				  if(dz1<-plist.zboxl/2.0){
				    cout <<"In sweep, current p1 protein interface below membrane: "<<p1<<" index: "<<iind1<<" loop: "<<it<<" dz: "<<dz1<<" iteration: "<<iter<<endl;
				    //write_crds_complex(bases[p1].mycomplex, ind_com, bases);
				  }
				  ...
				  if(dz2<-plist.zboxl/2.0){
				    cout <<"In sweep, current p2 protein interface below membrane: "<<p2<<" index: "<<iind2<<" loop: "<<it<<" dz2"<< dz2<<" iteration: "<<iter<<endl;
				    //write_crds_complex(bases[p2].mycomplex, ind_com, bases);
				  }
		...
		if (flag == 1) {
			...
			for (j = 0; j < t; j++) {
				...
				if ( movestat[p2] != 2) {
					/*
					 Proteins have already moved and avoided
		...
		if (it == maxit - 1) {
		  ...
		  if (k1 != k2) {
		    ...
		    cout <<"Reached max iterations in sweep without solving overlap, but proteins are within the same complex. Last separation: "<<sqrt(dr2)<<endl;
		  }
		  flags = 1;
		  saveit = maxit;
		  //exit(1);
