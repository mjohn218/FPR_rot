...
HARD CODED TO DEFINE REACTION PROBABILITY FOR LOOP CLOSURE TO BE BASED ON A0=300, self-binding, kr=mu[BACK_RXN], and pbound/punbound=Abound/Aunbound.
 */
	    ...
	    */
		...
		if (flagsep == 1 && kr[mu]>0) {
		  ...
		  if (myc1 != myc2) {
		    ...
		    /*Try using a detailed balance expression, where p_u->b=p_b->u *p_bound/p_unbound. 
		      p_b->u = 1-exp(-kb*deltat).
		      
		      Ratio of states. p_b/p_u=Keq/V gives overly low association probability.
		      
		    */
		    BIND_IN_SAME_COMPLEX=bindrad[mu];
		    if(sep<BIND_IN_SAME_COMPLEX){
		      V=plist.xboxl*plist.yboxl;
		      /*HARD CODED FOR A0=300, SELF_BINDING, k_b stored in mu=BACK_RXN*/
		       kb=kr[BACK_RXN];
		       Keq=ktemp/2.0/kb*1E6;//ASSUME SELF BINDING. units nm^3
		       A0=300;
		       Aunb=-0.25/Keq+0.5*sqrt(0.25/(Keq*Keq)+2.0*A0/V/Keq);
		       Aunb=Aunb*V;
		       Abound=(A0-Aunb);//account for all A's so get 1/2*2
		       /*pb_pu should be a function of Keq, and not D */
		       pb_pu=Abound/Aunb;//Keq/V;
		       probvec1=(1.0-exp(-1.0*kb*plist.deltat*1E-6))*pb_pu;
		...
		if (flagsep == 1 && kr[mu]>0) {
		  ...
		  */
		  if (myc1 != myc2) {
		    ...
		    BIND_IN_SAME_COMPLEX=bindrad[mu];
		    if(sep<BIND_IN_SAME_COMPLEX){
		       V=plist.xboxl*plist.yboxl*plist.zboxl;
		      /*HARD CODED FOR A0=300, SELF_BINDING, k_b stored in mu=BACK_RXN*/
		       kb=kr[BACK_RXN];
		       Keq=kact/2.0/kb*1E6;//ASSUME SELF BINDING
		       A0=300;
		       Aunb=-0.25/Keq+0.5*sqrt(0.25/(Keq*Keq)+2.0*A0/V/Keq);
		      Aunb=Aunb*V;
		       Abound=(A0-Aunb);//account for all A's so get 1/2*2
		      ...
		      //double pb_pu=Keq/V;
		      probvec1=(1.0-exp(-1.0*kb*plist.deltat*1E-6))*pb_pu;
		      //cout <<"IN EVALUATE BINDING PAIR DB (detailed Balance), set probvec1=: "<<probvec1<<endl;
